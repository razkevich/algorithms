...
- [x] **Time-based Key-Value Store (LC 981)** — `prep/design/TimeMap.java` (per-key `TreeMap<Integer, String>`; `floorEntry(timestamp)` is the whole problem. Pattern transfers to MVCC, snapshot isolation, time-series caches, "what was this at commit/timestamp X" queries.)
